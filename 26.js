/* Choose a color for an alien as you did in Exercise 25, and write an if-else chain.
• If the alien’s color is green, print a statement that the player just earned 5 points for shooting the
alien.
• If the alien’s color isn’t green, print a statement that the player just earned 10 points.
• Write one version of this program that runs the if block and another that runs the else block.
*/
var Alien_color = 'YELLOW';
// if else statement
if (Alien_color == 'green') {
    console.log('If the color of alien is green then the player will earn 5 points'); // true statement 
}
else {
    console.log('If the color of alien is not green then the player will earn 10 points'); // false statement 
}
var alien_color2 = 'red';
// if statement
if (alien_color2 == 'red') {
    console.log('If the color of alien is red then the player will earn 5 points'); // true statement 
}
else {
}
